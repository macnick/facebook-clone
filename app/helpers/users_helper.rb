  ...
  def toggle_button(user)
    if current_user != user
      if current_user.pending_friends.include?(user)
        button_to 'Cancel Request', { controller: 'friendships', action: 'destroy', friend_id: user }, method: :post, class: "form-control btn btn-danger mt-1", id:"button-#{user.id}"
        ...
        button_to 'Add Friend', { controller: 'friendships', action: 'destroy', friend_id: user }, method: :post, class: "form-control btn btn-success mt-1", id:"button-#{user.id}"
      end
    end
  end
end
